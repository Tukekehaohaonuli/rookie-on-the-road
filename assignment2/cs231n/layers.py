...
def batchnorm_backward_alt(dout, cache):
    ...
    dmean=np.sum(dx_hat*-1/np.sqrt(sample_var+eps),axis=0)+dvar*np.mean(-1*(x-sample_mean),axis=0)
...
def conv_backward_naive(dout, cache):
    ...
    for j in range(out_h):
        for k in range(out_w):
            ...
            w_coord=min(k*stride,W+2*pad-WW)
            for i in range(N):
                dx_pad[i,:,h_coord:h_coord+HH,w_coord:w_coord+WW]+=np.sum((dout[i,:,j,k])[:,None,None,None]*w,axis=0)
            for i in range(F):
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(out_h):
        for j in range(out_w):
            h_coord=min(stride*i,H-ph)
            w_coord=min(stride*j,W-pw)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(out_h):
        for j in range(out_w):
            h_coord=min(stride*i,H+ph)
            w_coord=min(stride*j,W-pw)
            x_max=np.max(x[:,:,h_coord:h_coord+ph,w_coord:w_coord+pw],axis=(2,3))
            mask=(x[:,:,h_coord:h_coord+ph,w_coord:w_coord+pw]==(x_max)[:,:,None,None])
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N,C,H,W=x.shape
    x_batch=x.transpose(0,2,3,1).reshape((N*H*W),C)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N,C,H,W=dout.shape
    dout_batch=dout.transpose(0,2,3,1).reshape((N*H*W),C)
    dx,dgamma,dbeta=batchnorm_backward(dout_batch,cache)
